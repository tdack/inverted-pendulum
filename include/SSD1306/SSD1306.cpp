    ...
    m_rst(rst), m_cs(cs), m_height(height)
{
  ...
  if (m_cs != NULL) m_cs->configure(pin::OUT);
...
SSD1306::reset(void)
{
  if (m_sclk != NULL) m_sclk->set(1);
  if (m_cs != NULL) m_cs->set(1);
...
SSD1306::refresh(void)
{
  ...
  for(int row = minRow; row <= maxRow; row++) {
    for(int col = minCol; col <= maxCol; col++) {
      SPIwrite(buffer[(get_width()*row)+col]);
    }
  }
...
SSD1306::getPixel(int16_t x, int16_t y)
{
  ...
  if ((buffer[x + (y/8)*get_width()] >> (7-(y%8))) & 0x1) return RGB::black;
  return RGB::white;
}
...
#define SPI
#ifdef SPI
  BeagleBone::SSD1306 drv(1, 0, // SPI
  ...
  for (x = 0, y = 0; x < X; x += dx, y += dy) {
    drv.drawPixel((int)x, (int)y, BeagleBone::RGB::black);
    drv.drawPixel((int)(drv.get_width() - x), (int)y, BeagleBone::RGB::black);
  }
  drv.refresh();
  ...
  BeagleBone::rgb_t color;

  X = drv.get_width() / 2 - 10;
  Y = drv.get_height() / 2 - 10;
  for (int i = 0; i < 11; i++) {
    color = (i%2) ? BeagleBone::RGB::white : BeagleBone::RGB::black;
    for (x = 0; x < 20; x++) {
      for (y = 0; y < 20; y++) {
