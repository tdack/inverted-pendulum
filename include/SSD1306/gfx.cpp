...
gfx::gfx(SSD1306::rgb_driver &drv) :
				m_drv(drv), m_auto_refresh(1), m_cursor_x(0), m_cursor_y(0), m_textcolor(
						RGB::black), m_textbgcolor(RGB::black), m_textsize(1), m_wrap(
								true) {
}

uint16_t gfx::getWidth(void) {
	return m_drv.get_width();
}

uint16_t gfx::getHeight(void) {
	return m_drv.get_height();
}
...
bool gfx::setAutoRefresh(bool is_on) {
	bool prev = m_auto_refresh;
	m_auto_refresh = is_on;
	return prev;
}

void gfx::clearScreen(void) {
	m_drv.clear();
	m_refresh();
	m_cursor_x = 0;
	m_cursor_y = 0;
...
void gfx::fillScreen(rgb_t color) {
	fillRect(0, 0, getWidth(), getHeight(), color);
	m_refresh();
}

// bresenham's algorithm - thx wikpedia
void gfx::drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		rgb_t color) {
...
void gfx::drawFastVLine(uint16_t x, uint16_t y, uint16_t h, rgb_t color) {
...
void gfx::drawFastHLine(uint16_t x, uint16_t y, uint16_t w, rgb_t color) {
...
void gfx::drawRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
		rgb_t color) {
...
void gfx::fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
		rgb_t color) {
	bool refrsh = setAutoRefresh(0);
...
void gfx::drawRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
...
void gfx::fillRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
		uint16_t r, rgb_t color) {
	bool refrsh = setAutoRefresh(0);
...
void gfx::drawCircle(uint16_t x, uint16_t y, uint16_t r, rgb_t color) {
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
	int16_t dx = 0;
...
void gfx::fillCircle(uint16_t x, uint16_t y, uint16_t r, rgb_t color) {
	bool refrsh = setAutoRefresh(0);
...
void gfx::drawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		uint16_t x2, uint16_t y2, rgb_t color) {
	bool refrsh = setAutoRefresh(0);
...
void gfx::fillTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		uint16_t x2, uint16_t y2, rgb_t color) {
	int16_t a, b, y, last;
...
void gfx::drawBitmap(uint16_t x, uint16_t y, const uint8_t *bitmap, uint16_t w,
...
void gfx::moveCursor(uint16_t x, uint16_t y) {
	m_cursor_x += x;
	m_cursor_y += y;
}

uint16_t gfx::getCursorX(void) {
	return m_cursor_x;
}

uint16_t gfx::getCursorY(void) {
	return m_cursor_y;
}
...
void gfx::setTextColor(rgb_t c) {
	m_textcolor = c;
	m_textbgcolor = c;
}

void gfx::setTextColor(rgb_t c, rgb_t b) {
	m_textcolor = c;
	m_textbgcolor = b;
}
...
void gfx::write(char c) {
	if (c == '\n') {
		...
		drawChar(c, m_cursor_x, m_cursor_y, m_textcolor, m_textbgcolor,
				m_textsize);
...
void gfx::m_refresh(void) {
	if (m_auto_refresh)
		m_drv.refresh();
}

void gfx::drawCircleHelper(uint16_t x, uint16_t y, uint16_t r, uint8_t corners,
		rgb_t color) {
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
...
void gfx::fillCircleHelper(uint16_t x, uint16_t y, uint16_t r, uint8_t corners,
		rgb_t color) {
...
void gfx::drawChar(uint8_t c, uint16_t x, uint16_t y, rgb_t color, rgb_t bg,
		uint8_t size) {
